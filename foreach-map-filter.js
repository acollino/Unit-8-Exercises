...
function doubleValues(arr) {
  let doubles = [];
  for (startingNum of arr) {
    doubles.push(startingNum * 2);
  }
  return doubles;
}
...
function onlyEvenValues(arr) {
  return arr.filter(function (startingNum) {
    return startingNum % 2 === 0;
  });
}
...
function showFirstAndLast(arr) {
  return arr.map(function (startingString) {
    let lastIndex = startingString.length - 1;
    return startingString[0] + startingString[lastIndex];
  });
}
...
function addKeyAndValue(arr, key, value) {
  return arr.map((startingObj) => {
    return { ...startingObj, [key]: value };
  });
}
...
function vowelCount(str) {
  let vowelArray = str.toLowerCase().match(/a|e|i|o|u/g);
  //could also use Array.from(str) and .filter for the vowels
  let vowelCounter = {};
  if (vowelArray != null) {
    vowelArray.forEach((vowel) => {
      //could also use for(vowel of vowelArray){}
      if (vowelCounter[vowel] == null) {
...
function doubleValuesWithMap(arr) {
  return arr.map((startingNum) => {
    return startingNum * 2;
  });
}
...
function valTimesIndex(arr) {
  return arr.map((startingNum, index) => {
    return startingNum * index;
  });
}

/*
Write a function called extractKey which accepts an array
 of objects and some key and returns a new array with the 
...
Write a function called extractFullName which accepts
 an array of objects and returns a new array with the value
  of the key with a name of "first" and the value of a key with the
...
function extractFullName(arr) {
  return arr.map((startingObj) => {
    return `${startingObj["first"]} ${startingObj["last"]}`;
  });
}
...
function filterByValue(arr, key) {
  return arr.filter(function (startingObj) {
...
function find(arr, searchValue) {
  let foundArray = arr.filter((startingNum) => {
    return startingNum == searchValue;
  });
  return foundArray[0];
}
...
function findInObj(arr, key, searchValue) {
  let foundArray = arr.filter((startingObj) => {
...
function removeVowels(str) {
  ...
  let noVowelArray = Array.from(lower).filter((letter) => {
    return (
      letter != "a" &&
      letter != "e" &&
      letter != "i" &&
      letter != "o" &&
      letter != "u"
      //could also use .match(regExp), just need to account for a null-return
    );
  });
...
array and returns a new array with all of the odd numbers doubled 
...
function doubleOddNumbers(arr) {
  let oddNumberArray = arr.filter((startingNum) => {
    return startingNum % 2 == 1;
  })
  return oddNumberArray.map((oddNum) => {
    return oddNum * 2;
  })
}
